...
pub fn run_dkg_dsg_t3_n5() -> JsValue {
    ...
    let secret_u256 =
        k256::U256::from_be_hex("0101010101010101010101010101010101010101010101010101010101010101");
    let secret_key = k256::Scalar::reduce(&secret_u256);

    // Run re_key (which includes DKG-like initialization) to get parties
    let (parties, _info) =
        re_key::<k256::Secp256k1>(&parameters, &session_id, &secret_key, None, |_| {
            String::new()
        });
    ...
    for &party_index in &executing_parties {
        ...
        all_data.insert(
            party_index,
            dkls23_core::protocols::signing::SignData {
    ...
    for &party_index in &executing_parties {
        let (session, transmit) = dkls23_core::protocols::sign_session::SignSession::new(
    ...
    obj.insert(
        "r".to_string(),
        serde_json::Value::String(hex::encode(signature.r)),
    );
    obj.insert(
        "s".to_string(),
        serde_json::Value::String(hex::encode(signature.s)),
    );
    obj.insert(
        "recov".to_string(),
        serde_json::Value::Number(serde_json::Number::from(signature.recovery_id as u8)),
    );
